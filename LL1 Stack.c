...
int main(){
	int size, i, j, k, isLL1 = 1, sizeOfTape;
	char input[15], tape[25];
	...
	{//LL1 Stack
		if(isLL1){
			printf("\nTHE GRAMMER IS LL1 GRAMMER.");
			
			printf("\nENTER STRING TO PARSE: ");
			scanf("%s",tape);
			
			sizeOfTape = strlen(tape);
			tape[sizeOfTape] = '$';
			tape[sizeOfTape+1] = '\0';
			sizeOfTape = strlen(tape);
			
			push('$');
			push(productionFront->lname);
			
			char topElement, cursor;
			
			i = 0;
			while(i<sizeOfTape){
				topElement = peek();
				cursor = tape[i];
				struct production* tempProd = isNonTerminal(topElement);
				if(tempProd != NULL){
					pop();
					for(j=0;j<sizeofTerminals;j++){
						if(terminals[j] == cursor) break;
					}
					if(j==sizeofTerminals || !strcmp(tempProd->terminalFirst[j], "NAN")){
						break;
					}
					else{
						for(k=strlen(tempProd->terminalFirst[j]);k>2;k--){
							if(tempProd->terminalFirst[j][k-1] != '#'){
								push(tempProd->terminalFirst[j][k-1]);	
							}
						}
					}
				} else {
					if(topElement == cursor){
						pop();
						i++;
					} else break;
				}
			}
			if(i==sizeOfTape) printf("VALID STRING");
			else printf("INVALID STRING");
		} else {
			printf("\nTHE GRAMMER IS NOT LL1 GRAMMER.");
		}
	}
	return 0;
}
